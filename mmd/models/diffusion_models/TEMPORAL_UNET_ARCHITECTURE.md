...
These smoke tests validated that the integration is operational. Full benchmark evaluations are now completed and documented in `CONTROLNET_EXPERIMENTS.md` Sections 2 to 5.

---

## Experiment Results and Follow-Up

`CONTROLNET_EXPERIMENTS.md` contains the benchmark plan, executed runs, aggregate analysis, and the `env_scale=1.5` stress-test.

- Completed experiments show that the simpler global FiLM-style adapter already improves planning most as scale and agent count increase.
- That result reduces the pressure to replace the current conditioning path immediately.
- Stronger spatial conditioning, especially Approach A / cross-attention from Section 6, is now a motivated next iteration rather than a speculative redesign.
- See `CONTROLNET_EXPERIMENTS.md` Sections 4 to 7 for the consolidated evidence and next-step recommendations.
